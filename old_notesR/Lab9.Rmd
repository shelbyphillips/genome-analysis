...
## Graphing confirmed cases for each US state
...
## organizing countries by most cases of corona virus
...
## Adding countries to confirmed covid-19 cases
...
## Several countries with confirmed covid-19 cases
...
## Cutoffs based on number of cases

```{r}
mybreaks <- c(1, 20, 100, 1000, 50000)
```

## Plotting covid-19 confirmed cases from 3.11.20
...
## Adaptation of code from Prof. Chris Sunderland: Get data, pivot to longer format and summarize
...
## Confirmed COVID-19 Cases in 3 Portuguese speaking countries
Data is not available as of 3/11/20 for other Portuguese speaking countries
```{r}
time_series_confirmed_long %>% 
  filter (Country.Region %in% c("Portugal", "Brazil", "Angola")) %>%
    ggplot(aes(x = Date,  y = Confirmed, color = Country.Region)) + 
    geom_point() +
    geom_line() +
    ggtitle("Confirmed COVID-19 Cases")
...
## Confirmed COVID-19 Cases in some countries of the Middle East

```{r}
time_series_confirmed_long %>% 
  filter (Country.Region %in% c("Jordan", "Algeria", "Lebanon", "Kuwait")) %>%
    ggplot(aes(x = Date,  y = Confirmed, color = Country.Region)) + 
    geom_point() +
    geom_line() +
    ggtitle("Confirmed COVID-19 Cases")
...
## Confirmed cases for each province in China
...
report_03_11_2020 %>% 
  ...
  ggplot(aes(x = Confirmed,  y = reorder(Province.State, Confirmed))) + 
    geom_point() +
    ggtitle("Confirmed cases for each province in China") +
    ylab("Country/Region") +
    xlab("Confirmed Cases")
```
...
report_03_11_2020 %>% 
  ...
  ggplot(aes(x = Confirmed,  y = reorder(Province.State, Confirmed))) + 
    geom_point() +
    ggtitle("Confirmed cases for each province in Australia") +
    ylab("Country/Region") +
    xlab("Confirmed Cases")
```
